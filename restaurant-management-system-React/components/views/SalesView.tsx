...
import { SalesAnalytics, Settings } from '../../types';
...
const SalesView: React.FC<SalesViewProps> = ({ 
  analytics, settings, onExportSales, selectedDate, onDateChange, availableDates 
}) => {
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between gap-4">
        <div>
            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                <BarChart3 className="text-green-600" /> تقرير المبيعات
            </h2>
            <p className="text-sm text-gray-500 mt-1">تحليل أداء المطعم لليوم المحدد</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-green-100 relative overflow-hidden">
            ...
            <p className="text-3xl font-bold text-gray-800">R$ {analytics.totalSales.toFixed(2)}</p>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-blue-100 relative overflow-hidden">
            <p className="text-sm text-gray-500 font-medium mb-1">الطلبات المكتملة</p>
            <p className="text-3xl font-bold text-gray-800">{analytics.ordersCount}</p>
            {analytics.cancelledCount > 0 && <span className="text-xs text-red-500 bg-red-50 px-2 py-0.5 rounded-full mt-2 inline-block">ملغاة: {analytics.cancelledCount}</span>}
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-orange-100 relative overflow-hidden">
            <p className="text-sm text-gray-500 font-medium mb-1">عدد التوصيلات</p>
            <p className="text-3xl font-bold text-gray-800">{analytics.deliveryCount}</p>
            <p className="text-xs text-orange-600 mt-1">تكلفة: R$ {analytics.deliveryCost.toFixed(2)}</p>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-purple-100 relative overflow-hidden">
            <p className="text-sm text-gray-500 font-medium mb-1">الربح الصافي</p>
            <p className={`text-3xl font-bold ${analytics.netProfit >= 0 ? 'text-purple-600' : 'text-red-600'}`}>R$ {analytics.netProfit.toFixed(2)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">💳 تفاصيل الدفع</h3>
            <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-green-50/50 rounded-xl">
                    <span className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-500"></span> نقداً (Dinheiro)</span>
                    <span className="font-bold font-mono">R$ {analytics.paymentBreakdown.cash.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-blue-50/50 rounded-xl">
                    ...
                    <span className="font-bold font-mono">R$ {analytics.paymentBreakdown.pix.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-indigo-50/50 rounded-xl">
                    <span className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-indigo-500"></span> بطاقة (Cartão)</span>
                    <span className="font-bold font-mono">R$ {analytics.paymentBreakdown.cartao.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-red-50/50 rounded-xl">
                    ...
                    <span className="font-bold font-mono">R$ {analytics.paymentBreakdown.ifood_card.toFixed(2)}</span>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h3 className="font-bold text-gray-800 mb-4">📍 المصادر</h3>
                <div className="flex flex-wrap gap-2">
                    {Object.entries(analytics.sourceBreakdown).map(([key, val]) => (
                        <span key={key} className="px-3 py-1 bg-gray-100 rounded-lg text-sm text-gray-700 font-medium flex items-center gap-2">
                            {key === 'salon' ? 'الصالة' : key === 'whatsapp' ? 'واتساب' : key === 'ifood' ? 'iFood' : key}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h3 className="font-bold text-gray-800 mb-4">🍽️ الأنواع</h3>
                <div className="flex flex-wrap gap-2">
                    {Object.entries(analytics.typeBreakdown).map(([key, val]) => (
                        <span key={key} className="px-3 py-1 bg-gray-100 rounded-lg text-sm text-gray-700 font-medium flex items-center gap-2">
                            {key === 'dine_in' ? 'محلي' : key === 'takeaway' ? 'سفري' : 'توصيل'}
                            <span className="bg-white px-2 py-0.5 rounded text-xs shadow-sm">{val}</span>
                        </span>
                    ))}
                </div>
      ...
      {/* Items Sold */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h3 className="font-bold text-gray-800 mb-4">🍔 تفاصيل الأصناف المباعة</h3>
        {Object.keys(analytics.itemsSold).length === 0 ? (
            <div className="text-center py-8 text-gray-400">لا توجد مبيعات مسجلة لهذا اليوم</div>
        ) : (
                        ...
                        {Object.entries(analytics.itemsSold)
                            ...
                            .map(([name, data]) => {
                                ...
                                <tr key={name} className="group hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="py-3 font-mono text-green-600">R$ {d.total.toFixed(2)}</td>
                                </tr>
                                );
                            })}
                    </tbody>
